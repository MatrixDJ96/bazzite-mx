...
| 15 | `ujust install-discord` fails after Bazzite rebases to a new Fedora release | Vendored GPG key is named `RPM-GPG-KEY-rpmfusion-nonfree-fedora-44` (literal `44`). The repo file references `gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-rpmfusion-nonfree-fedora-$releasever`. After a Bazzite rebase to F45, `$releasever` expands to `45` and the lookup misses our vendored key. | At every Bazzite Fedora-major rebase: re-extract the new key via `dnf5 install rpmfusion-nonfree-release-$NEW_RELEASE` in a base-image podman, and replace the vendored `RPM-GPG-KEY-rpmfusion-nonfree-fedora-XX` filename. The `.repo` files don't need to change (they use `$releasever`). Add a smoke test assertion comparing key filename suffix vs `rpm -E %fedora` output if needed | Phase 8 review (caught proactively before any rebase) |
