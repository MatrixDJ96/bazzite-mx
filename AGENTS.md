...
1. **Bash**: `#!/usr/bin/env bash`, `set -euo pipefail`; clean under
   ...
   - **Form** (`docs/conventions.md` § Bash → Form): every script touched, new or edited,
     leaves in that form: `if … then … fi` flow, one action per line, 100 columns, names
     that say what they do, a header with usage and exit status, comments only where they
     carry a contract or a reason. `.github/scripts/check-form.sh` checks the shapes and
     the width; the edit hook runs it on every shell file an edit touches. The same rules
     hold for every other file of the repo, workflows and configuration included.
...
6. **Conventional Commits**, one commit per verified feature (script + test + doc together),
   every message with the shape of `docs/conventions.md` § Commits: `<type>(<scope>): <what>`
   within 72 columns, a body that says what and why, no trailer; `check-commits.sh` checks
   the whole history in the lint job.
...
8. **CI naming**, the style of `ublue-os/bazzite`'s workflows: workflow `name:` Title Case;
   a job name is the phase in one Title Case word (`Lint`, `Build`, `Gate`; the matrix job of
   the reusable build is the flavour); a step name is Title Case, verb + object, no article,
   tools in their own casing (`Install Cosign`, `Push to GHCR`); env vars
   `SCREAMING_SNAKE_CASE`; outputs `snake_case`, the same key everywhere;
