...
// The bottom panel of the primary screen, else any panel of it, else null.
function primaryPanel() {
    const all = panels();
    const bottom = all.find(p => p.screen === 0 && p.location === "bottom");
    if (bottom) {
        return bottom;
    }
    return all.find(p => p.screen === 0) || null;
}

// The pinned launchers of the panel's task manager, "" without one.
