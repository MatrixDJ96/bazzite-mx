...
│   ├── kmods/                   # Out-of-tree kernel module builder (see note below)
...
`build_files/kmods/` is built in a dedicated `kmod-builder` Containerfile stage, NOT by `build-mx.sh` (whose
`[0-9]*-*.sh` glob only runs `build_files/mx/`). It compiles out-of-tree kernel modules (`msi-ec`) against
`kernel-devel` from the `akmods:ogc-<fedora>-<kver>` carrier and stages the `.ko` for the final stage, which
installs it into `updates/` and runs `depmod`.
...
`group: bazzite-mx-{stream}-${{ github.ref_name }}`) means in-flight runs are
