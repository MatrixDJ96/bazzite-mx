...
| Script | Role |
|---|---|
| `lib.sh` | coordinates, `exit_with_error`/`print_error`, `emit`, `read_env`, `image_of`, `TAG_SHAPE`; sourced by all |
...
| `release-tag.sh <coords> \| --tag <tag> <coords>` | the release tag, `.N` only when the tag is taken; a forced tag, refused when taken |
...
| `check-form.sh <file>...` | the form rules (§ Bash → Form of `conventions.md`): line width and the banned control-flow shapes |
...
| `00-prep.sh` | dnf keeps its cache; the base's repository files, enabled set and recipe sets are recorded |
...
| `usr/lib/bazzite-mx/host.sh` | what `verify-host`, `migrate` and `ntfsplus-setup` read about the host (`host_file`, the sources, the residue lists) and the `exit_with_error`/`require_root` they share |
