...
substitution changes nothing and prints nothing: the step's `Adding /dev/kvmfr0 to qemu
cgroup_device_acl` runs, the file stays as it was. The rewrite of 2026-09-06 kept the edit as
upstream wrote it; a fix is a behaviour change, filed for its own round.
...
kernel-modules feature. Capture the output in a variable, then grep the variable. The shape
can pass for months and fail on a base change: `tests/95-clean-stage.sh` read the kernel lock
through `dnf5 versionlock list | grep -q` and was green while the list held five entries; the
base of 2026-09-07 (`44.20260907`, kernel 7.2.3-ogc3.1) locks every qt6 and plasma package
too, the list runs to 3179 lines, and the three flavours went red on `tests: FAILED` with a
...
`tests/30-ide.sh`, five lines of output, on a base the main run had passed minutes earlier:
in the base's container the pipe failed 16 times in 1000 runs. Every `| grep -q` of the repo
now captures first and `check-form.sh` refuses the shape.
...
too. `ntfs_alias_resolves` in `bazzite-mx-ntfsplus-setup` (then `alias_resolves`) used `-n -v` and read a working opt-in as
a failed mask on every host whose driver was already loaded: on the desktop, opt-in active and
four volumes mounted, it returned 1 with an empty dry run (measured 2026-09-06). The function
now reads `--show-depends` after the captured configuration. The note of 2026-09-05 that
`--show-depends` printed `insmod` through an active blacklist did not reproduce in any cell of
the table. The same probe once read the blacklist through `! modprobe -c | grep -q`, the
SIGPIPE shape of § `command | grep -q` above on a 2 MB config, so its negation was never false:
the config is captured first.
...
push the run list is read before anything is assumed, and what is missing is dispatched by hand,
`gh workflow run build.yml --ref <branch>`, and `deploy-pages.yml` likewise.
...
`src/lib/data/releases.json`, which runs from 1.0.0 to 1.3.3 (re-read 2026-09-06 at the pinned
commit). Any other version fails with "official ORAS CLI releases does not contain version
1.3.4" (measured 2026-09-03 on both flavours, after the push and the signature of `:staging`).
The list on the action's `main` reaches 1.3.4 without a release. `install-oras.sh` installs
from the ORAS release directly, the tarball refused unless its sha256 matches the release's
checksums file.
...
`release_tag` ([`workflow.md`](workflow.md)). `cleanup.sh` deletes only names that never come
back. Measured 2026-09-06 22:18Z, every release, tag, GHCR version and run deleted: the version
job refused to pick a tag from an empty registry (`no tag found … refusing to pick a tag`),
which is why an empty answer now counts as "nothing taken" and only a failed probe stops it.
...
where the GHCR logins already live; anything else a job installs on the host goes the same way.

## An unclean stop of a self-hosted host leaves its container storage missing layers

A runner host that loses power or hangs mid-build leaves the podman storage the job was
writing with layers that the image index still lists and the overlay directory no longer has.
The next job on that instance dies in seconds at the base pull with `unknown layer: Stat
.../overlay/<id>/diff: no such file or directory`, a message that reads like a registry
problem. Measured 2026-09-06 after a host stopped without a shutdown sequence during a
main-profile run: that run went red as a host failure, and the first job after the restart
failed on one instance's storage. `podman system check --quick` on an idle instance's storage
finds the missing layers without hashing every blob, and `--repair` removes the layer and the
images that use it, the bases downloading again on the next job (podman-system-check(1)).
After any unclean stop of the host, every instance's storage is checked before the runners
take a job again.
