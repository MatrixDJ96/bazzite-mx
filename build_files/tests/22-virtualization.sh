...
# formatting names readonly (libformatting.sh: b, n, bold, normal); an
# assignment to one of them fails at every run. The readonly list is read
# from the image, and an empty list means the probe cannot see, not a pass.
readonly_names=$(grep -hoE '^declare -r [A-Za-z_]+' /usr/lib/ujust/*.sh | awk '{ print $3 }' | sort -u)
assigned=$(grep -vE '^[[:space:]]*#' "$HELPER" | grep -oE '(^|[[:space:]])[A-Za-z_][A-Za-z0-9_]*=' | tr -d ' =' | sort -u)
clash=$(grep -xF -f <(echo "$readonly_names") <<< "$assigned" || true)
if [ -z "$readonly_names" ]; then
    echo "FAIL: no readonly name found under /usr/lib/ujust (the ujust libraries changed shape?)"
elif [ -z "$clash" ]; then
    echo "OK: kvmfr helper assigns none of the $(wc -l <<< "$readonly_names") names ujust.sh declares readonly"
else
