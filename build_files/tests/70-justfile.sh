...
# Known-bad: a feed without the checksum link, a link naming another build, a
# checksum file that is not a sha256, a tarball with another top directory, a
# binary without the execute bit. Each is refused with nothing under the home.
refuse_install() { # <feed> <message> <what>
    rm -rf "$tb/home/.local" "$tb/home/.cache"
    if out=$(run_toolbox "$1" install 2>&1); then
        echo "FAIL: toolbox installer accepted $3"
    elif grep -q "$2" <<< "$out" && [ ! -e "$tb/home/.local/share/JetBrains/ToolboxApp" ]; then
        echo "OK: toolbox installer refuses $3"
    else
        echo "FAIL: toolbox installer on $3: $(tail -n1 <<< "$out")"
    fi
}
rm -rf "$tb/jetbrains-toolbox-9.9.9"
mkdir -p "$tb/jetbrains-toolbox-9.9.9/bin"
printf '#!/bin/sh\necho toolbox\n' > "$tb/jetbrains-toolbox-9.9.9/bin/jetbrains-toolbox"
chmod 755 "$tb/jetbrains-toolbox-9.9.9/bin/jetbrains-toolbox"
printf '9.9.9\n' > "$tb/jetbrains-toolbox-9.9.9/bin/build.txt"
tar czf "$tb/jetbrains-toolbox-9.9.9.tar.gz" -C "$tb" jetbrains-toolbox-9.9.9
...
printf '{"TBA":[{"build":"9.9.9","downloads":{"linux":{"link":"file://%s/jetbrains-toolbox-9.9.9.tar.gz"}}}]}\n' "$tb" > "$tb/feed-noshape.json"
printf '{"TBA":[{"build":"9.9.8","downloads":{"linux":{"link":"file://%s/jetbrains-toolbox-9.9.9.tar.gz","checksumLink":"file://%s/good.sha256"}}}]}\n' "$tb" "$tb" > "$tb/feed-otherbuild.json"
feed garbage.sha256 > "$tb/feed-garbage.json"
refuse_install feed-noshape.json 'the feed lost its shape' 'a feed without the checksum link'
refuse_install feed-otherbuild.json 'does not name build 9.9.8' 'a link that names another build'
refuse_install feed-garbage.json 'does not start with a sha256' 'a checksum file that is not a sha256'
mv "$tb/jetbrains-toolbox-9.9.9" "$tb/toolbox-9.9.9"
tar czf "$tb/jetbrains-toolbox-9.9.9.tar.gz" -C "$tb" toolbox-9.9.9
mv "$tb/toolbox-9.9.9" "$tb/jetbrains-toolbox-9.9.9"
printf '%s *jetbrains-toolbox-9.9.9.tar.gz\n' "$(sha256sum "$tb/jetbrains-toolbox-9.9.9.tar.gz" | cut -d' ' -f1)" > "$tb/good.sha256"
refuse_install feed-good.json 'unexpected top directory' 'a tarball with another top directory'
chmod 644 "$tb/jetbrains-toolbox-9.9.9/bin/jetbrains-toolbox"
tar czf "$tb/jetbrains-toolbox-9.9.9.tar.gz" -C "$tb" jetbrains-toolbox-9.9.9
printf '%s *jetbrains-toolbox-9.9.9.tar.gz\n' "$(sha256sum "$tb/jetbrains-toolbox-9.9.9.tar.gz" | cut -d' ' -f1)" > "$tb/good.sha256"
refuse_install feed-good.json 'is not executable' 'a tarball whose binary is not executable'
rm -rf "$work"
